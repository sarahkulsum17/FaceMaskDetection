...
# Mask Detector Model - with custom_objects to handle compatibility issues
try:
    ...
    # Define custom objects with our custom class
    custom_objects = {'DepthwiseConv2D': CustomDepthwiseConv2D}
    
    # Load the model with custom objects
    Mask_Model = load_model('artifacts/Mask_detection_model.h5', custom_objects=custom_objects)
...
def draw_rectangle(img_path, locs, preds):
    ...
    for i, face_loc in enumerate(locs):
        ...
        # Replace deprecated textsize method with newer approach
        try:
            ...
            try:
                ...
                # If all else fails, use estimated dimensions
                text_width = len(text) * 7  # Rough estimate
                text_height = 15  # Rough estimate
